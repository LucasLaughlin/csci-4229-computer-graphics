...
static void hat(double x, double y, double z, double r)
{
   ...
   for (float r = 0; r <= 3; r += step)
   {
      ...
      for (int angle = 15; angle <= 375; angle += diff)
      {
         ...
         if (yv >= yv_brim-1.5)
         {
            iv = -0.5 * pow(xv, 3) - 0.5 * xv * pow(zv, 2) + 0.9 * pow(xv, 2);
            jv = -1;
            kv = -2 * pow(zv, 3) - 0.5 * zv * pow(xv, 2) + 1.8 * pow(zv, 2);
         ...
         {
            iv = -0.5 * pow(xv_step, 3) - 0.5 * xv_step * pow(zv_step, 2) + 0.9 * pow(xv_step, 2);
            jv = -1;
            kv = -2 * pow(zv_step, 3) - 0.5 * zv_step * pow(xv_step, 2) + 1.8 * pow(zv_step, 2);
            mag = sqrt(pow(iv, 2) + pow(jv, 2) + pow(kv, 2));
         }
         else
         {
