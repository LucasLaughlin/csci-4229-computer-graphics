...
static void Wall(float x,float y,float z, float th,float ph , float Sx,float Sy,float Sz , float St)
{
   ...
   mat4 ModelMatrix, rot1;
   //  Transform
   glm_mat4_identity(ModelMatrix);
   glm_translate(ModelMatrix, (vec3){x,y,z});
   glm_rotate_x(ModelMatrix, ph*3.1415926/180, rot1);
   glm_rotate_y(rot1, th*3.1415926/180, ModelMatrix);
   ...
   int id = glGetUniformLocation(shader[shader_mode], "ModelMatrix");
   if (id >= 0)
   ...
   glDrawArrays(GL_TRIANGLE_STRIP, 0, bufIndexCount / 3-1);
...
static void Hat(double x, double y, double z, double r)
{
   ...
   glm_mat4_identity(ModelMatrix);
   glm_translate(ModelMatrix, (vec3){x,y,z});
   glm_scale(ModelMatrix, (vec3){r, r, r});
   ...
   glUniformMatrix3fv(glGetUniformLocation(shader[shader_mode], "NormalMatrix"), 1, 0, (float *)NormalMatrix);
...
void Scene(){
   Hat(0,0,0,1.0);
   Wall(0,-1,0, 0,-90 , 8,8,6 , 4);
}

void ConfigureLightPOV(GLuint shader){
   float near_plane = 1.0f, far_plane = 7.5f;
   mat4 LightProjectionMatrix, LightViewMatrix, LightSpaceMatrix;
   glm_ortho(-10.0f, 10.0f, -10.0f, 10.0f, near_plane, far_plane, LightProjectionMatrix);
   ...
   // T 
   glm_mat4_mul(LightProjectionMatrix, LightViewMatrix, LightSpaceMatrix);
   int id;
...
void SetTransformMatrices(int shader_index)
{
   mat4 ViewMatrix;
   Light(0);
   ...
   //  Set matrices
   glUniformMatrix4fv(glGetUniformLocation(shader[shader_index], "ModelViewMatrix"), 1, 0, ModelViewMatrix);
   glUniformMatrix4fv(glGetUniformLocation(shader[shader_index], "ViewMatrix"), 1, 0, (float *)ViewMatrix);
   glUniformMatrix4fv(glGetUniformLocation(shader[shader_index], "ProjectionMatrix"), 1, 0, ProjectionMatrix);
   //glUniformMatrix3fv(glGetUniformLocation(shader, "NormalMatrix"), 1, 0, (float *)NormalMatrix);
   glUniform4fv(glGetUniformLocation(shader[shader_index], "LightPos"), 1, LightPos);
   glUniform4fv(glGetUniformLocation(shader[shader_index], "ViewPos"), 1, ViewPos);
}

void ConfigureShadowShader(int shader_index){
   glUniform1i( glGetUniformLocation(shader[shader_index], "ShadowMap"), depthMapFBO);
   glUniform1i(glGetUniformLocation(shader[shader_index], "DiffuseTexture"), tex);
}
...
void ShadowMap(){
   ...
   ConfigureLightPOV(5);
   SetTransformMatrices(5);
   ConfigureShadowShader(5);
...
void display()
{
   ...
   if(shader_mode==5){
      ...
      SetTransformMatrices(shader[shader_mode]);
      int id;
